...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-[100dvh] w-full relative overflow-hidden flex flex-col items-center transition-colors duration-1000 ease-in-out ${isDarkMode ? 'bg-gradient-to-b from-[#1a1a2e] to-[#16213e]' : 'bg-gradient-to-b from-[#ff9a9e] to-[#fad0c4]'}`}>

      {/* Header */}
      <div className="w-full max-w-md p-4 pt-8 flex justify-between items-center z-10">
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col justify-center items-center w-full z-10 pb-10 relative">
        ...
        {/* Teddy */}
        <div className="mb-8 relative transform transition-all duration-500 scale-100 sm:scale-105">
          {/* Speech Bubble */}
          {speechText && (
            <div className="absolute -top-24 left-1/2 -translate-x-1/2 bg-white dark:bg-gray-100 text-gray-800 px-5 py-3 rounded-2xl shadow-xl border-2 border-white animate-pop-in z-50 whitespace-nowrap after:content-[''] after:absolute after:top-full after:left-1/2 after:-translate-x-1/2 after:border-8 after:border-transparent after:border-t-white dark:after:border-t-gray-100">
        ...
        {isReadingNote && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in" onClick={() => setIsReadingNote(false)}>
            <div onClick={(e) => e.stopPropagation()} className="bg-[#fff0f5] dark:bg-gray-800 w-[90%] max-w-sm max-h-[80dvh] overflow-y-auto rounded-3xl p-6 sm:p-8 shadow-2xl text-center relative border-4 border-pink-200 animate-pop-in transform transition-all custom-scrollbar">
              <button onClick={() => setIsReadingNote(false)} className="absolute top-3 right-3 p-2 bg-white/50 rounded-full hover:bg-white/80 transition z-10"><X size={20} className="text-gray-500" /></button>
              <div className="text-4xl sm:text-5xl mb-4 sm:mb-6">💌</div>
              <p className="font-handwriting text-base sm:text-lg text-gray-800 dark:text-gray-100 leading-relaxed font-display whitespace-pre-wrap">
                {SWEET_MESSAGE}
              </p>
